...
- Implementations for most basic regular use for android app. Only one major functionality TODO from android side, send scaner locaiton information to sever, will work on this one later in the evening. 
- Basic theories are very clear now, still a fair amount of work to do, also needs the sever in order to test. Will continue to work on this one on 4/21 evening when the group meets. 
...
- Spend a few hours today to review for tomorrow evening's data structure test. Then work on either android app (cube app, or debug tetris). 
...
- After the one leftover functionality being done, will work on android music cube app for java programming course as a individual course project. 

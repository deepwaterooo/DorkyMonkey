...
- Signin needs to call server, now StrickMode it works, thread method works too now. left over similiar sever calls are all considered labor-work, will work on them when I am tired. will work on different functionalities, for example. the confirm views before/after QRCode scan. work on screen sizes first though. 
- suggested views: 
  - During 4/21/16 evening meeting, a view of QRCode information (date, course, course_code & check type: In/Out, together with a confirm button) are suggested to add before server call, I am curretly thinking will implement this using fragments in camera activity
  - SignIn are not required at signin activity phase, but needs to be checked later any way, so maybe I should still discuss with the team that indication of signin successful or not view (or a piece of information) needs to be added.
  - Report is tomorrow evening, I could still work on it tonight or tomorrow, but need to record a video before tomorrow evening's course review. 
- Studio is restrict on images, all the four images that teammates want do NOT work with studio. will try this later only when all other functionalities are done;
- have basic Fragment and layout for different screen sizes, suggested by teammates, will Potentially try to fit different screen sizes. 
- 
